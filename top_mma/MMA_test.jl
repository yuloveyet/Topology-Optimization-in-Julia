...
function mmasub(m::Int, n::Int, iter::Int, xval::Array{Float64}, xmin::Array{Float64},
    xmax::Array{Float64}, xold1::Array{Float64}, xold2::Array{Float64}, f0val::Float64,
    ...
    # dgdx2 = P * Diagonal(uxinv3) + Q * Diagonal(xlinv3)
    # dgdx2 = 2.0 * dgdx2
    # del = dfdx2 - dgdx2
    # delpos = zeros(m, n)
...
function subsolv(m::Int, n::Int, epsimin::Float64, low::Array{Float64}, upp::Array{Float64},
    ...
    while epsi > epsimin    # Start while epsi>epsimin
        ...
        ittt = 0
        while (residumax > 0.9 * epsi) && (ittt < 100) # Start while (residumax>0.9*epsi) and (ittt<200)
            ittt = ittt + 1
            itera = itera + 1
            ...
            GG = P .* Diagonal(uxinv2) - Q .* Diagonal( xlinv2)
            dpsidx = (plam ./ ux2) - (qlam ./ xl2)
            ...
            if m < n # Start if m < n
                ...
                Alam = Diagonal(diaglamyi) + (GG .* Diagonal(diagxinv)) * transpose(GG)
                AAr1 = [Alam a]
                ...
                Axx = Diagonal(diagx) + (transpose(GG) .* Diagonal(diaglamyiinv)) * GG
